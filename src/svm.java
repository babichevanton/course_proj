...
public class svm {
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			else
			{
				...
				for(j=begin;j<end;j++)
				{
					...
					//svm_predict_values(submodel,prob.x[perm[j]],dec_value,confidence);
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target)
	{
		...
		for(i=0;i<nr_fold;i++)
		{
			    ...
			    param.svm_type == svm_parameter.NU_SVC))
			{
				...
				{
					//Double[] confidence = new Double[1];
					//target[perm[j]] = svm_predict(submodel,prob.x[perm[j]],confidence);
	...
	//public static double svm_predict_values(svm_model model, svm_node[] x, double[] dec_values, Double[] confidence)
	public static double svm_predict_values(svm_model model, svm_node[] x, double[] dec_values)
	{
		...
		else
		{
			...
			//confidence[0] = (double) vote[0];
			int vote_max_idx = 0;
			for(i=1;i<nr_class;i++)
				if(vote[i] > vote[vote_max_idx])
				{
					vote_max_idx = i;
					//confidence[0] = (double) vote[i];
				}
			//confidence[0] /= nr_class*(nr_class-1)/2;

			return model.label[vote_max_idx];
		}
	}

	//public static double svm_predict(svm_model model, svm_node[] x, Double[] confidence)
	public static double svm_predict(svm_model model, svm_node[] x)
	{
		...
		//double pred_result = svm_predict_values(model, x, dec_values, confidence);
	...
	public static double svm_predict_probability(svm_model model, svm_node[] x, double[] prob_estimates)
	{
		    ...
		    model.probA!=null && model.probB!=null)
		{
			...
			//Double[] confidence = new Double[1];
			//svm_predict_values(model, x, dec_values, confidence);
		...
		{
			//Double[] confidence = new Double[1];
			//return svm_predict(model, x, confidence);
